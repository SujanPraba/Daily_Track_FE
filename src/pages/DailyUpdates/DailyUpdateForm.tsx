...
import { Calendar, Clock, FileText, Building2, Ticket, Loader2, ChevronLeft } from 'lucide-react';
...
const DailyUpdateForm: React.FC<DailyUpdateFormProps> = ({
  ...
  // Ticket timing state
  const [ticketHours, setTicketHours] = useState('0.00');
  ...
  useEffect(() => {
    if (dailyUpdate) {
      ...
      // Set ticket hours if available
      if (dailyUpdate.tickets) {
        setTicketHours('0.00'); // Default to 0.00 for existing updates
      }
    } else {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.tickets.trim()) {
      newErrors.tickets = 'Tickets are required';
    }

    // Validate ticket hours are numeric and positive
    if (parseFloat(ticketHours) < 0) {
  ...
  const handleInputChange = (field: keyof CreateDailyUpdateDto, value: string) => {
    ...
    // Handle ticket hours change
  const handleTicketHoursChange = (value: string) => {
    setTicketHours(value);
  };
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  min="0"
                  value={ticketHours}
                  onChange={(e) => handleTicketHoursChange(e.target.value)}
                  placeholder="0.00"
                  className="block w-full pl-10 pr-3 py-2 rounded-lg border-gray-300 shadow-sm focus:ring-orange-500 focus:border-orange-500 hover:border-orange-400 transition-colors"
                />
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="0.00"
                className={`block w-full pl-10 pr-3 py-2 rounded-lg border-gray-300 shadow-sm focus:ring-orange-500 focus:border-orange-500 hover:border-orange-400 transition-colors ${
                  errors.internalMeetingHours ? 'border-red-500' : ''
                }`}
              />
              <Clock className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
            </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="0.00"
                className={`block w-full pl-10 pr-3 py-2 rounded-lg border-gray-300 shadow-sm focus:ring-orange-500 focus:border-orange-500 hover:border-orange-400 transition-colors ${
                  errors.externalMeetingHours ? 'border-red-500' : ''
                }`}
              />
              <Clock className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
            </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="0.00"
                className={`block w-full pl-10 pr-3 py-2 rounded-lg border-gray-300 shadow-sm focus:ring-orange-500 focus:border-orange-500 hover:border-orange-400 transition-colors ${
                  errors.otherActivityHours ? 'border-red-500' : ''
                }`}
              />
              <Clock className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" />
            </div>
